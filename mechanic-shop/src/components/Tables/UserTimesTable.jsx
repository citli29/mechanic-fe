...
export default function UserTimesTable({
	...
	const [newUserTime, setNewUserTime] = useState(emptyUserTime);

	const [newUserTimePunch, setNewUserTimePunch] = useState(emptyUserTimePunch);
	...
	async function loadUserTimes() {
		try {
			const res = await api.get(`/services/${serviceId}/user_times`);
			setUserTimes( res.data.sut_list || []);
		} catch (err) {
			setUserTimes([]);
			handleError(err);
		}
	...
	async function loadUserTimePunches() {
		try {
			const res = await api.get(`/services/${serviceId}/user_time_punches`);
			setUserTimePunches( res.data.sutp_list || []);
		}
		catch (err) {
			setUserTimePunches([]);
			handleError(err);
	...
	function cancelEditingPunch(){setEditingPunch(null);}
	...
	async function createUserTime() {
		...
		setSaving(true);
		try {
			await api.post(`/services/${serviceId}/user_times`,{
					user_id: Number(newUserTime.user_id),
					minutes: Number(newUserTime.minutes),
	...
	async function saveUserTime() {
		if (!validateUserTime(editing)) { return; }

		setSaving(true);
		try {
			await api.put(`/services/${serviceId}/user_times/${editing.sut_id}`,
				{
					user_id: Number(
						editing.user_id
					),
	...
	async function deleteUserTime(userTime) {
		...
		const confirmed = window.confirm( `Delete the time entry for "${userTime.user_name}"?`);
		if (!confirmed) {return;}
	...
	async function createUserTimePunch() {
		...
		if (!validateUserTimePunch(newUserTimePunch)) { return; }
		setPunchSaving(true);

		try {
			await api.post( `/services/${serviceId}/user_time_punches`, 
				{
					user_id: Number( newUserTimePunch.user_id),
					date: newUserTimePunch.date
				}
			);

			showMessage?.( "success", "Registo de ponto criado com sucesso.");
			setCreatingPunch(false);
			setNewUserTimePunch(emptyUserTimePunch);
	...
	async function saveUserTimePunch() {
		...
		if (!validateUserTimePunch(editingPunch)) { return;}

		setPunchSaving(true);

		try {
			await api.put(`/services/${serviceId}/user_time_punches/${editingPunch.sutp_id}`,
				{
					user_id: Number( editingPunch.user_id),
					date: editingPunch.date
				}
			);

			showMessage?.("success","Registo de ponto atualizado com sucesso.");
			setEditingPunch(null);
	...
	async function deleteUserTimePunch(userTimePunch) {
		const confirmed = window.confirm( `Apagar o registo de ponto de "${userTimePunch.user_name}"?`);
		if (!confirmed) {return;}

		setPunchAction({id: userTimePunch.sutp_id, type: "delete"});

		try {
			await api.delete( `/services/${serviceId}/user_time_punches/${userTimePunch.sutp_id}`);
			showMessage?.( "success", "Registo de ponto apagado com sucesso.");

			if ( editingPunch?.sutp_id === userTimePunch.sutp_id) {
				setEditingPunch(null);
			}
			await loadUserTimePunches();
		} catch (err) { handleError(err);
	...
	async function startUserTimePunch(userTimePunch) {
		...
		setPunchAction({id: userTimePunch.sutp_id, type: "start"});

		try {
			await api.post( `/services/${serviceId}/user_time_punches/${userTimePunch.sutp_id}/start`);
			showMessage?.( "success", "Ponto iniciado com sucesso.");
			await loadUserTimePunches();
		} catch (err) { handleError(err);
		} finally { setPunchAction(null);}
	}
	...
	async function stopUserTimePunch(userTimePunch) {
		...
		setPunchAction({ id: userTimePunch.sutp_id, type: "stop"});

		try {
			await api.post( `/services/${serviceId}/user_time_punches/${userTimePunch.sutp_id}/stop`);
			showMessage?.( "success", "Ponto terminado com sucesso.");
			await loadUserTimePunches();
		} catch (err) { handleError(err);
		} finally { setPunchAction(null);}
	}
			...
			<div className="table-wrapper">
				<table className="table">
					...
					<tbody>
						...
						{!loading && userTimes.length === 0 && !creating && (
							<tr>
								<td colSpan={4}>Sem Tempos de Serviço.</td>
							</tr>
						)}

						{!loading && userTimes.map(userTime => {
							const isCurrentEditing = editing?.sut_id === userTime.sut_id;
							if (isCurrentEditing) {
								return (
									<tr key={userTime.sut_id} className="editing">
										<td>
											<select
												name="user_id"
							...
							return (
								<tr key={userTime.sut_id}>
									<td>{userTime.user_name || "-"}</td>
									<td> {userTime.minutes} </td>
									<td> {formatDateDisplay(userTime.date)} </td>
									<td>
										<div className="container-buttons">
											<button
												type="button"
											...
											<button
												...
												onClick={() => deleteUserTime(userTime)}
											>
			...
			<div className="table-wrapper ">
				<table className="table">
					...
					<tbody>
						...
						{!punchesLoading && userTimePunches.length === 0 && !creatingPunch && (
							<tr>
								<td colSpan={6}> Sem Registos de Ponto. </td>
							</tr>
						)}

						{!punchesLoading && userTimePunches.map(userTimePunch => {
								const isCurrentEditing = editingPunch?.sutp_id === userTimePunch.sutp_id;
								const isStarting = punchAction?.id === userTimePunch.sutp_id 
									&& punchAction?.type === "start";

								const isStopping = punchAction?.id === userTimePunch.sutp_id 
									&& punchAction?.type === "stop";

								const isDeleting = punchAction?.id === userTimePunch.sutp_id 
									&& punchAction?.type === "delete";

								const isActionRunning = punchAction?.id === userTimePunch.sutp_id;

								const hasStarted =
									userTimePunch.hours_s !== null && userTimePunch.minutes_s !== null;

								const hasFinished =
									userTimePunch.hours_f !== null && userTimePunch.minutes_f !== null;
								...
								if (isCurrentEditing) {
									return (
										...
										>
											...
											<td>
												{hasStarted ? formatPunchTime( userTimePunch.hours_s, userTimePunch.minutes_s) : "-"}
											</td>

											<td>

												{hasFinished ? formatPunchTime( userTimePunch.hours_f, userTimePunch.minutes_f) : "-"}
											</td>

											<td> {userTimePunch.minutes ?? "-"} </td>
								...
								return (
									<tr key={userTimePunch.sutp_id}>
										<td> {userTimePunch.user_name || "-"} </td>
										<td>
											{hasStarted ? ( 
												formatPunchTime(userTimePunch.hours_s, userTimePunch.minutes_s)
											) : (
												<button
													type="button"
													onClick={() =>
										...
										<td>
											{hasFinished ? (
												formatPunchTime( userTimePunch.hours_f, userTimePunch.minutes_f)
											) : (
												<button
													type="button"
													onClick={() =>
										...
										<td> {userTimePunch.minutes ?? "-"} </td>
										<td> {formatDateDisplay( userTimePunch.date)} </td>
										<td>
											<div className="container-buttons">
												<button
													type="button"
